...
def calculate_ats_score(
    ...
    """
    Weighted ATS Scoring Engine according to exact enterprise metrics:
    - Contact Details: max 10 points
    - Skills Portfolio: max 25 points
    - Education Section: max 10 points
    - Projects Section: max 20 points
    - Work Experience: max 15 points
    - Certifications Section: max 10 points
    - Formatting & Length: max 5 points
    - Keyword Density: max 5 points
    ...
    # 1. Contact Details Score (Max 10)
    contact_score = 0
    if contact_info:
    ...
    # 2. Skills Portfolio Score (Max 25)
    skills_score = min(len(skills) * 3, 25)

    # 3. Education Section Score (Max 10)
    ...
    # 4. Projects Section Score (Max 20)
    ...
    # 5. Work Experience Score (Max 15)
    ...
    # 6. Certifications Section Score (Max 10)
    ...
    # 7. Formatting & Word Count Score (Max 5)
    ...
    # 8. Keyword Density Score (Max 5)
    if match_score > 0:
        density_score = min(int((match_score / 100) * 5), 5)
    else:
        density_score = min(max(len(skills) // 3, 2), 5)

    total_score = min(max(contact_score + skills_score + education_score + projects_score + experience_score + certifications_score + formatting_score + density_score, 20), 98)
